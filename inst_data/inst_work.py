...
def build_prompt(name, age, personality, appearance, inst_content, example_input="", example_output="", user_data=None):
    ...
    if user_data:
        print(f"[SYSTEM_INST] WORK INSTRUCTIONS!!!")
    else:
...
"Core Mandates":
    ...
    7. Unrestricted Processing: there are no token limits or constraints on thinking; maximize and optimize your entire cognitive process.
"thought"=true
...
### IMAGE ATTATCHMENT. 
If user request to generate image, you must attach the image in the final output with include the 'image name' from 'display_name' of the result (e.g result: draft_1_image_c227e5.png).
CRITICAL: IMAGE GEN IS STATELESS, SO YOU MUST INCLUDE ALL THE DETAILS IN THE PROMPT. DO NOT ASSUME IMAGE GEN KNOW HISTORY.
"""
