...
async def run_pipeline(context_pack: Dict[str, Any]) -> AsyncGenerator[str, None]:
    """
    Text discussion pipeline. Receives all pre-computed data from logic_router.
    ...
    rp_core._debug_print("TO_TEXT", f"Pipeline started | Intent: {context_pack.get('detected_intent')}")
    ...
    if attachment_filenames:
        rp_core._debug_print("TO_TEXT", f"Attachments available: {attachment_filenames}")
    ...
    while loop_count < loop_limit:
        ...
        while retry_count <= max_retries:
            ...
            if error_encountered:
                rp_core._debug_print("TO_TEXT", f"Stream error: {error_encountered}")
                if retry_count < max_retries:
                    retry_count += 1
                    rp_core._debug_print("TO_TEXT", f"Retrying {retry_count}/{max_retries}...")
                    await asyncio.sleep(2)
                else:
                    if has_generated_image:
                        rp_core._debug_print("TO_TEXT", "Max retries reached. Using fallback model response.")
                        # FALLBACK RESPONSE
        ...
        if has_critical_error:
            rp_core._debug_print("TO_TEXT", f"Critical tool error, stopping pipeline: {critical_error_msg}")
            yield json.dumps({"type": "error", "content": critical_error_msg}) + "\n"
            break
            
        ctx_api = [] 

    rp_core._debug_print("TO_TEXT", "Pipeline finished.")
